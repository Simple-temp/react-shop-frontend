...
const OrderScreen = () => {
    ...
    const onApprove = (data, actions) => {
        return actions.order.capture().then(async function (details) {
            try {
                ...
                const { data } = await axios.put(`https://website-12.herokuapp.com/api/orders/${order._id}/pay`,
                    details,
                    {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            try {
                ...
                const { data } = await axios.get(`https://website-12.herokuapp.com/api/orders/${orderId}`, {
                    headers: { authorization: `Bearer ${userInfo.token}` }
                })
        ...
        if (!order._id || successPay || (order._id && order._id !== orderId)) {
            ...
            const loadPaypalScript = async () => {
                const { data: clientId } = await axios.get("https://website-12.herokuapp.com/api/key/paypal", {
                    headers: { authorization: `Bearer ${userInfo.token}` }
                })
                paypalDispatch({
                    type: "resetOptions",
    ...
    const onToken = async (token) => {
        ...
        try {
            const { data } = await axios.put(`https://website-12.herokuapp.com/api/orders/${order._id}/stripe`,
                {
                    headers: { authorization: `Bearer ${userInfo.token}` },
                    token,
                },
